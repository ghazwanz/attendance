...
import { handleEditName } from '../../actions/handleEdit';
...
const EditName = ({ name, onClose, onSuccess }: EditNameProps) => {
    const [state, action, pending] = useActionState(handleEditName, null)
    ...
    return (
        <div className='flex flex-col w-full gap-3'>
            <h4 className='text-lg'>Edit Nama Lengkap</h4>
            <form action={action} className='space-y-5'>
                <div>
                    <label htmlFor="name" className="dark:text-zinc-300 hidden text-zinc-700 text-sm mb-3">
                        Nama Lengkap
                    </label>
                    <input
                        type="text"
