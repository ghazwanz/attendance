...
export default function Tabeljadwal() {
    ...
    const handleUpdate = async (updatedItem: Schedule) => {
        const { error } = await supabase
            .from('schedules')
            .update({ day: updatedItem.day, start_time: updatedItem.start_time, end_time: updatedItem.end_time })
    ...
    return (
        <>
                    ...
                    <tbody>
                        {data.map((schedule, index) => (
                            ...
                            >
                                ...
                                <td className="px-6 py-4 text-yellow-500 font-semibold">{schedule.start_time}</td>
                                <td className="px-6 py-4 text-blue-500 font-semibold">{schedule.end_time}</td>
                                <td className="px-6 py-4 space-x-2">
                                    <button
                                        onClick={() => { setSelectedItem(schedule); setShowEdit(true); }}
                                    ...
                                    <button
                                        onClick={() => { setSelectedItem(schedule); setShowDelete(true); }}
            ...
            {showDelete && selectedItem && (
                <DeleteModal item={selectedItem} onClose={() => setShowDelete(false)} onConfirm={() => handleDelete(selectedItem)} />
            )}

            {showEdit && selectedItem && (
                <EditModal item={selectedItem} onClose={() => setShowEdit(false)} onSave={handleUpdate} />
            )}
        </>
    );
}
